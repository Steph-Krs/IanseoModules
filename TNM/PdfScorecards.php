...
$pdf->PrintBye=($_REQUEST['IncBye']??0);
...
if (safe_num_rows($Rs)>0) {
	...
	while($MyRow=safe_fetch($Rs)) {
		set_time_limit(30);
		if(empty($_REQUEST["Blank"]) && empty($_REQUEST["IncEmpty"]) && (empty($MyRow->M1Athlete) || empty($MyRow->M2Athlete)) && !$pdf->PrintBye) {
			continue;
		}
		...
		if(empty($_REQUEST["IncEmpty"]) and (!$MyRow->M1Athlete or !$MyRow->M2Athlete) && !$pdf->PrintBye) {
			continue;
		}
		...
		!$MyRow->M1Athlete ? $pdf->AddPage() : DrawScore($pdf, $MyRow, 'L');

		// Disegna lo score di destra
		!$MyRow->M2Athlete ? '' : DrawScore($pdf, $MyRow, 'R');
		...
		if(!empty($_REQUEST['Barcode']) && !$isBye) {
		...
		if(!empty($_REQUEST['QRCode']) && !$isBye) {
