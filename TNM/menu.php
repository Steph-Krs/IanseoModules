...
// TNM secondary nav bar — injected on every page for TNM competitions.
// menu.php is glob-included by Ianseo's get_which_menu() on all pages;
// output lands inside #navigation (body) with clear:both so it appears
// as a second row below the float:left <li> menu items.
if (empty($GLOBALS['_tnm_nav_done']) && !empty($_SESSION['TourId'])) {
